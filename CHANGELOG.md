...
- File type and size are checked on the opened file rather than the path, so a file cannot be swapped for a FIFO or a larger file between the check and the read. `acd init` and `--output` create files exclusively (`O_EXCL`). An existing file is opened without truncation and only overwritten after confirming the path still names that same regular file, so there is no gap between a symlink check and the write on any platform.
